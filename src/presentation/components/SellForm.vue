...
const canSubmit = computed(() => {
  ...
  return productData.value.images.length > 0 &&
...
const updateSizeOptions = () => {
  ...
  // Limpiar talla si cambia la categoría
  productData.value.size = '';
};

const handleCancel = () => {
  // Redirigir a la página principal o a mis productos
...
const submitProduct = async () => {
  ...
  if (!acceptTerms.value) {
    formError.value = 'Debes aceptar los términos para publicar tu producto';
    return;
  }
...
<template>
  <div class="max-w-2xl mx-auto bg-white rounded-[2rem] shadow-lg p-4 sm:p-6 lg:p-8">
    <!-- Header -->
    <div class="mb-8">
      <h1 class="text-2xl sm:text-3xl font-black text-gray-900 mb-2">Vende tu tesoro</h1>
      <p class="text-gray-600 text-sm sm:text-base">Completa el formulario para publicar tu producto</p>
    </div>
    ...
    <form @submit.prevent="submitProduct" class="space-y-6">
      ...
      <div class="pt-6 space-y-3">
        ...
        >
          ...
          {{ authLoading ? 'Verificando...' : isSubmitting ? 'Publicando...' : 'Publicar producto' }}
        </button>
